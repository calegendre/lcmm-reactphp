...
// Function to make API calls to Radarr
function make_api_request($url, $method = 'GET', $data = null) {
    ...
    curl_setopt_array($curl, [
        ...
        CURLOPT_USERPWD => RADARR_AUTH_USER . ':' . RADARR_AUTH_PASS
    ]);
...
function handle_movies($user_id) {
    // Get all movies from Radarr
    $response = make_api_request(RADARR_URL . '/api/v3/movie');
...
function handle_rootfolders() {
    ...
    $response = make_api_request(RADARR_URL . '/api/v3/rootfolder');
...
function handle_search($user_id) {
    ...
    $response = make_api_request(RADARR_URL . '/api/v3/movie/lookup?term=' . urlencode($term));
    ...
    $existing_response = make_api_request(RADARR_URL . '/api/v3/movie');
