...
<?php
// sonarr.php - Sonarr API proxy endpoints
...
switch ($action) {
    case 'series':
        handle_series($request_method, $user_id);
        break;
...
function handle_series($method, $user_id) {
    if ($method !== 'GET') {
...
function handle_search($method, $user_id) {
    ...
    $response = make_api_request(
        SONARR_URL . '/api/v3/series/lookup?term=' . urlencode($term), 
        SONARR_API_KEY,
        SONARR_AUTH_USER,
        SONARR_AUTH_PASS
    );
